...
function customAssign(...objects) {
  ...
  const objBig = Object.assign(...objects);
  for (const value in objBig) {
    if (typeof objBig.value === "number") {
      objBig.value = String(objBig.value);
    }
    return objBig;
  }
}
...
function customObjectFromEntries(entries) {
  ...
  const checking = entries.map(([key, value]) => {
    if (typeof key === "number") {
      value = String(key);
    }
    return [key, value];
  });
  const newObj = Object.fromEntries(checking);
  return newObj;
}
...
function checkProperty(obj, prop) {
  ...
  const checking = obj.hasOwnProperty(prop);
  return checking;
}
...
function extendAndFreezeObject(obj1, obj2) {
  ...
  const objUnion = Object.assign(obj1, obj2);
  const objFroz = Object.freeze(objUnion);
  if (Object.freeze(objUnion)) {
    return objFroz;
  } else {
    return "Error";
  }
...
function keysAndValues(obj) {
  ...
  const objValu = Object.values(obj);
  return [objKey, objValu];
}
...
function checkAndPreventExtensions(obj) {
  ...
  const checkObj = Object.isExtensible(obj);
  return checkObj;
}
...
function sealAndCheck(obj) {
  ...
  Object.seal(obj);
  const sealedObj = Object.isSealed(obj);
  return sealedObj;
}
...
function getObjectValuesSum(obj) {
  ...
  const sumObj = valuesObj.reduce((sum, num) => sum + num, 0);
  return sumObj;
}
